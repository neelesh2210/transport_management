        ...
        <div class="col-12">
          ...
          <div class="card">
            <div class="card-header">
              ...
              @can('truck-create')
              <a href="{{ route('truck.create') }}" class="btn btn-primary btn-info" style="float:right;"><i class="fas fa-plus" aria-hidden="true"></i>  Add</a>
              @endcan
            </div>
              ...
              <table id="example1" class="table table-bordered table-striped">
               ...
               @foreach($data as $item)
                ...
                <td>{{$item->vechile_number}}</td>
                  <td>{{$item->vechile_type}}</td>
                  @php $own=App\VechileOwner::where('id', $item->owner_id)->first()  @endphp 
                  <td>{{$own->ownwer_name}}</td>
                  <td>{{$item->gross_capicity}}</td>
                  <td>{{$item->normal_load}}</td>
                  <td>{{$item->unladen_weight}}</td>
                <td>
            <label class="switch">
            ...
            <span class="slider round"></span>
                  ...
                  <td>
                   <div class="btn-group">
       @can('truck-edit') 
       <a href="{{route('truck.edit',$item->id)}}" class="btn btn-primary btn-sm"><i class="fas fa-pencil-alt" aria-hidden="true"></i> Edit</a>
       @endcan
       @can('truck-delete')
     <form action="{{ route('truck.destroy', $item->id) }}" method="POST">
    @method('DELETE')
    @csrf
