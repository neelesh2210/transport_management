         ...
         @if(Session::get('data')!='')
          ...
          <div class="card">
            <div class="card-header">
              ...
              @can('loading-slip-create')
              <a href="{{ route('loading_slip.create') }}" class="btn btn-primary btn-info" style="float:right;"><i class="fas fa-plus" aria-hidden="true"></i>  Add</a>
              @endcan
            </div>
            ...
            <div class="card-body">
              <table id="example1" class="table table-bordered table-striped">
               ...
               @foreach($data as $item)
                <td>{{$i++}}</td>
                <td>{{$item->di_no}}</td>
                @php $truck=App\Truck::where('id', $item->truck_no)->first()  @endphp 
            ...
            <span class="slider round"></span>
                  ...
                  <td>
                   <div class="btn-group">
       <a href="{{route('loading_slip.show',$item->id)}}" class="btn btn-primary btn-sm"><i class="fas fa-view" aria-hidden="true"></i>View</a> 
       @can('loading-slip-edit')
       <a href="{{route('loading_slip.edit',$item->id)}}" class="btn btn-primary btn-sm"><i class="fas fa-pencil-alt" aria-hidden="true"></i> Edit</a>
       @endcan
       @can('loading-slip-delete')
     <form action="{{ route('loading_slip.destroy', $item->id) }}" method="POST">
    @method('DELETE')
    @csrf
