...
<section class="content">
      <div class="row">
         ...
         @if(Session::get('data')!='')
          ...
          <div class="card">
            <div class="card-header">
              ...
              @can('fuel-create')
              <a href="{{ route('fuel.create') }}" class="btn btn-primary btn-info" style="float:right;"><i class="fas fa-plus" aria-hidden="true"></i>  Add</a>
              @endcan
            </div>
            ...
            <div class="card-body">
              <table id="example1" class="table table-bordered table-striped">
               ...
               @foreach($data as $item)
                <td>{{$i++}}</td>
                 @php $petrol=App\PetrolPump::where('id', $item->petrol_pump)->first()  @endphp 
                <td>{{$petrol->petrolpump_name}}</td>
        ...
        <a href="{{route('fuel.show',$item->id)}}" class="btn btn-primary btn-sm"><i class="fas fa-view" aria-hidden="true"></i> View</a>
        @can('fuel-edit')
       <a href="{{route('fuel.edit',$item->id)}}" class="btn btn-primary btn-sm"><i class="fas fa-pencil-alt" aria-hidden="true"></i> Edit</a>
       @endcan
       @can('fuel-delete')
     <form action="{{ route('fuel.destroy', $item->id) }}" method="POST">
    @method('DELETE')
    @csrf
