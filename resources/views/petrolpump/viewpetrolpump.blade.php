...
<section class="content">
      <div class="row">
         ...
         @if(Session::get('data')!='')
          ...
          <div class="card">
            <div class="card-header">
              <h3 class="card-title">Petrol Pump List</h3>
            ...
            <div class="card-body">
              <table id="example1" class="table table-bordered table-striped">
               ...
               @foreach($data as $item)
                <td>{{$i++}}</td>
                <td>{{$item->petrolpump_name}}</td>
                <td>{{$item->petrolpump_address}}</td>
                
                @php $branch=App\Companie::where('id', $item->branch)->first()  @endphp
                 <td>{{$branch->company_branch_name}} ({{$branch->company_branch_code}})</td>
                <td>
            <label class="switch">
       ...
       <a href="{{route('petrol_pump.edit',$item->id)}}" class="btn btn-primary btn-sm"><i class="fas fa-pencil-alt" aria-hidden="true"></i> Edit</a>
       @endcan
       @can('petrol-pump-delete')
     <form action="{{ route('petrol_pump.destroy', $item->id) }}" method="POST">
    @method('DELETE')
    @csrf
